...
src_install() {
	exeinto /sbin
	doexe "${S}"/sub-projects/zramconfig/zramconfig
	doman "${S}"/sub-projects/zramconfig/man/zramconfig.1 || die
	dodoc "${S}"/{Changelog,README} || die

	newinitd "${FILESDIR}/init.d-${PN}" ${PN%config} || die
	newconfd "${FILESDIR}/conf.d-${PN}" ${PN%config} || die
}
